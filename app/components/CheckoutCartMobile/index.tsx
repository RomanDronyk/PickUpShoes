...
const CheckoutCartMobile = ({cartsFromCart}: any) => {
  ...
  const variantUrl = useVariantUrl(
    cartsFromCart.merchandise.product.handle,
    cartsFromCart.merchandise.selectedOptions,
  );

  return (
    <div className="flex flex-col  relative  justify-center  register max-h-[250px] overflow-hidden   pt-[24px] mb-[30px] lg:mb-0">
      <div className="grid min-w-full grid-cols-1 relative justify-between items-center gap-0">
        <div className="flex gap-[16px]   pb-[7px]">
          <div className="max-w-[70px] relative block rounded-[20px] overflow-hidden group-hover/card:h-[calc(var(--image-height)-var(--options-height)+10px)] w-full h-full transition-all duration-100 ease-in-out">
            <Link
              to={variantUrl}
              className="relative overflow-hidden max-w-[70px]"
              style={{height: 'var(--image-height)'}}
            >
              <Image
                alt={
                  cartsFromCart?.merchandise?.image?.altText ||
        ...
        <div>
          <div className="pb-[12px]">
            <OptionList
              sku={cartsFromCart?.merchandise?.sku}
              options={cartsFromCart?.merchandise?.selectedOptions}
            />
          </div>
          <div>
            <h4 className="md:text-xl text-lg font-semibold line-clamp-1 pr-[10px] mb-[7px]">
