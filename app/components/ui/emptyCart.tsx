  ...
  const {cartShow, setCartShow}  = useContext(HeaderBasketContext) as HeaderContextInterface
    ...
    return (
      <div className="flex flex-col gap-5 min-h-52 h-full items-center justify-center">
        ...
        <Button onClick={setOpen? ()=>setOpen(false): ()=> setCartShow(false)} asChild className="bg-red font-medium hover:bg-darkRed text-xl">
          <Link onClick={setOpen? ()=>setOpen(false): ()=> setCartShow(false)}  to="/collections/catalog">
