...
const GoogleAuthButton = () => {
  ...
  const [customer, setCustomer] = useState<{ id: string, email: string, accessToken: string } | any>(null);
  ...
  const handleCustomEvents = useCallback((event: any) => {
    if (["login", "activate", "multipass"].includes(event.detail.action)) {
      ...
      console.log(event.detail.current, "detail customer")
    }
  }, []);
        ...
        {isClient &&
          <>
            <SocialLoginWidget
              ...
              logout={(e: any) => { console.log(e) }}
              refresh={(e: any) => { console.log(e) }}
            />
          </>
        }
      </div>
