  ...
  return [
    {
      ...
      'Content-Security-Policy':
        "default-src 'self' https://cdn.shopify.com https://shopify.com http://localhost:*; img-src 'self' https://imagedelivery.net https://cdn.shopify.com https://shopify.com data: blob:; style-src 'self' 'unsafe-inline'; media-src *; script-src 'self' 'unsafe-eval';",
    },
  ];
};
...
export const action: ActionFunction = async ({context, request, params}) => {
  ...
  if (content.length < 5) {
    errors.content = 'Відгук має бути не менше 5 букв';
  } else if (!content) {
    errors.content = 'Вкажіть текст';
  }
...
export async function loader(args: LoaderFunctionArgs) {
  ...
  const getReviews = await fetchProductReviewsEasy(
    getProductId[getProductId.length - 1],
  );
...
export default function Product() {
  ...
  return <ProductPageScreen loaderData={loaderData} />;
}
