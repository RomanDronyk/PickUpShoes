...
export const action = async ({request, context}: ActionFunctionArgs) => {
  ...
  return new Response(
    JSON.stringify({success: true, likedProducts: updatedLikes}),
    {
      status: 200,
      headers: {
...
export async function loader({context, request}: any) {
  ...
  return defer(
    {
      bestSellers: filterAvailablesProductOptions(
...
export default function Liked() {
  ...
  const {setLikedCardId} = useContext(
    HeaderBasketContext,
  ...
  return (
    <div
      className="w-full  flex flex-col  lg:px-24 md:px-12 px-[10px]  mb-8"
      style={{margin: '0 auto'}}
    >
      ...
      <div
        className={
          isMobile
            ? 'flex flex-col min-h-[100px] relative  '
            : 'flex flex-col min-h-[100px] relative  justify-center  register rounded-[20px] border border-black/10 p-6 my-[12px] mb-[34px]'
        }
      >
        {products.length > 0 ? (
          <div style={{display: 'grid', gap: 18}}>
            {products.map((product: {productVariant: IVariant}) => (
              <LikedCart
