...
export async function loader({ request, params, context }: LoaderFunctionArgs) {
  ...
  const appliedFilters = filters.filter((filter) => !filter.available).map((filter) => {
  ...
  return json({
    collection: {
      ...
      products: {
        ...collection.products,
        filters: collection.products.filters.filter((filter) => filter.id !== "filter.v.availability"),

        nodes: filterAvailablesProductOptions(collection.products.nodes),
      },
    },
    headerPromise,
    filtersCollection,
...
export default function Collection() {
  ...
  const memoizedInitialFilters = useMemo(
    () => filtersCollection?.products.filters,
    [filtersCollection?.products.filters],
  );
      ...
      <div className="items relative">
        ...
        <Pagination connection={collection.products}>
          {({
            ...
            <>
              <PreviousLink>
                <Button className="m-[0_auto]" variant={'outline'}>
                  {isLoading ? (
                    'Загрузка...'
                  ) : (
              ...
              <NextLink ref={ref}>
                <Button className="m-[0_auto]" variant={'outline'}>
                  {isLoading ? 'Загрузка...' : <span>↓ Наступна сторінка</span>}
                </Button>
              </NextLink>
            </>
          )}
